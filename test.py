    ...
    def run(self):
        ...
        async def listen():
            try:
                ...
                channel = client.channel("public-db-changes")
...
class LoginDialog(QDialog):
    def __init__(self, parent=None):
        ...
        title_lbl = QLabel("Enrollment Authentication")
        ...
        btn_login = QPushButton("Login to Enroll")
...
class ClassroomDashboard(QWidget):
    ...
    def __init__(
        ...
        if (
            ...
            p5_start = QTime(13, 25)
            p5_end = QTime(14, 25)
            p5_str = "01:25 PM - 02:25 PM"
        else:
            p5_start = QTime(13, 45)
            p5_end = QTime(14, 25)
        ...
        if self.class_num >= 11:
            self.full_schedule_structure.append(
                (
                    "9",
                    p9_str,
                    p9_start,
        ...
        supabase_url = os.environ.get(
            "SUPABASE_URL",
            "https://grhcviuhtvuebfdznlpn.supabase.co"
        )

        supabase_key = os.environ.get(
            "SUPABASE_KEY",
            "sb_secret_Bl98A7QqcYBZ8bGnp_KS0g_VDRREXOL"
        )

        self.realtime_thread = RealtimeListenerThread(
            supabase_url,
            supabase_key,
            self
        )
    ...
    def open_cal(self):
        try:
            ...
            QMessageBox.warning(
                self,
                "Calendar Error",
                f"Could not open the Academic Calendar.\n\n{e}"
            )
    ...
    def open_facial_attendance(self):
        ...
        self.attendance_window = FacialAttendanceWindow(
            back_callback=self.return_from_facial_attendance,
            source_class=self.selected_class_name
        )
    ...
    def return_from_facial_attendance(
        ...
        if (
            hasattr(self, "attendance_window")
        ...
        if source_class:
            self.selected_class_name = source_class

            self.class_title.setText(
                f"Classroom Portal - {self.selected_class_name}"
            )
        ...
        if (
            ...
            try:
                ...
                QMessageBox.warning(
                    self,
                    "Enrollment Error",
                    f"Could not open the Student Enrollment page.\n\n{e}"
            )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if os.path.exists(admin_path):
                subprocess.Popen(
                    ...
                    cwd=os.path.dirname(admin_path)
                )
    ...
    def update_notices_button_style(self):
        ...
        seen_count = (
            ClassroomDashboard.seen_notices_counts.get(
                self.selected_class_name,
                0
            )
        )
    ...
    def update_live_time_and_status(self):
        ...
        current_routine_day = day_map[
            py_weekday
        ]

        selected_routine = self.all_routines.get(
            self.selected_class_name,
            {}
        )

        today_classes = selected_routine.get(
            current_routine_day,
            {}
        )
        ...
        if current_routine_day in [1, 7]:
            ...
            if active_block:
                ...
                if is_break:
                    ...
                    subj, teacher = today_classes.get(
                        p_idx,
                        (
                            "Free Period",
                            "N/A"
    ...
    def init_ui(self):
        self.setWindowTitle(
            f"SOS Hermann Gmeiner School Gandaki "
